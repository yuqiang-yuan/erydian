...
use gpui_kit::{
    App, AppContext, Entity, Window, base::input::{InputState, TextareaState}, component::select::{SearchableVec, SelectState},
};
...
impl AttrState {
    pub fn value(&self, cx: &App) -> AttrValue {
        match self {
            ...
            AttrState::MultilineText(state) => AttrValue::Text(Some(state.read(cx).value().to_string())),
    ...
    pub fn from_kind(kind: &AttrKind, window: &mut Window, cx: &mut App) -> AttrState {
        match kind {
            AttrKind::Text { default , multiple_line: false } => AttrState::Text(cx.new(|cx| {
                InputState::new(window, cx).default_value(default.clone().unwrap_or(String::new()))
            })),

            AttrKind::Text { default , multiple_line: true } => AttrState::MultilineText(cx.new(|cx| {
                TextareaState::new(window, cx)
                    .rows(5)
                    .default_value(default.clone().unwrap_or(String::new()))
            })),
